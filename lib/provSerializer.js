...
function addEntityRecord(store, entityCollection, entity, subject) {
    var props = entityCollection[entity];
    store.addTriple(subject, "rdfs:label", "\"" + props["prov:label"] + "\"@en");
    if (props.hasOwnProperty("dct:identifier")) {
        store.addTriple(subject, "dct:identifier", "\"" + props["dct:identifier"] + "\"");
    }
    if (props.hasOwnProperty("foaf:name")) {
        store.addTriple(subject, "foaf:name", "\"" + props["foaf:name"] + "\"");
    }
    if (props.hasOwnProperty("foaf:mbox")) {
        var email = props["foaf:mbox"];
        if (email.startsWith("<") && email.endsWith(">"))
            store.addTriple(subject, "foaf:mbox", email);
        else
            store.addTriple(subject, "foaf:mbox", "<" + email + ">");
    }
}
...
function addPrefixTriples(store, provObject) {
    ...
    store.addTriple("@prefix", "foaf:", "<http://xmlns.com/foaf/0.1/>");
    store.addTriple("@prefix", "dct:", "<http://purl.org/dc/terms/>");
