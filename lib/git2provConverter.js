...
function getCommitObj(line) {
    ...
    // 5  - author date %ad
    // 6  - committer name %cn
    // 7  - committer email %ce
    // 8  - committer date %cd
    // 9  - subject %s
    // 10 - modification type &

    commitObj.entity = "file-" + data[0].replace(/ /g, "-");
    commitObj.rawId = data[1];
    ...
    commitObj.author_email = data[4];
    commitObj.author_date = new Date(data[5]).toISOString();
    commitObj.commiter = "user-" + data[6].replace(/ /g, "-");
    commitObj.commiter_label = data[6];
    commitObj.commiter_email = data[7];
    commitObj.commiter_date = new Date(data[8]).toISOString();
    commitObj.subject = data[9];
    commitObj.modification_type = data[10];
    return commitObj;
}
...
function updateProvObj(provObject, urlprefix, commitObj){
    ...
    provObject.activities[urlprefix + ":" + commitObj.id] = {
        "prov:label" : commitObj.subject,
        "dct:identifier" : commitObj.rawId
    };// No need to add the parents as well, since we will eventually loop over them anyway
    ...
    provObject.agents[urlprefix + ":" + commitObj.author] = {
        "a" : "foaf:Person",
        "prov:label" : commitObj.author_label,
        "foaf:mbox": "<mailto:"+commitObj.author_email+">",
        "foaf:name": commitObj.author_label
    };
    ...
    provObject.agents[urlprefix + ":" + commitObj.commiter] = {
        "prov:label" : commitObj.commiter_label,
        "foaf:mbox": "<mailto:"+commitObj.commiter_email+">",
        "foaf:name": commitObj.commiter_label
    };
...
function getPrefixes(urlprefix, requestUrl, serialization) {
    ...
    var queryPos = requestUrl.indexOf('&');
    var url = (queryPos !== -1) ? requestUrl.substring(0, queryPos) : requestUrl;
    prefixes.fullResult = url + "&serialization=" + serialization + "#";
    return prefixes;
}
...
function getLogCmds(provObject, fileList, urlprefix, options) {
    ...
    var logCmds = files.map(function (file) {
        ...
        return 'git --no-pager log --date=iso --name-status --pretty=format:"' + currentEntity + ',' + commitHash + ',' + parentHash + ',%an,%ae,%ad,%cn,%ce,%cd,%s,&" -- ' + file;
    });
    return logCmds;
}
